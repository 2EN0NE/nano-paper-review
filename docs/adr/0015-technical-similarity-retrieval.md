...
- **集合算法**：Overlap 覆盖度 `|subject特征 ∩ reference特征| / |reference特征|`（方向性：Reference 的技术方法是否落在 Subject 技术范围内）。二值命中（不加权），特征先归一化（去空格/小写 + 剥前后缀泛称黑名单）再做前缀匹配（短名 vs 全称），对齐「同一技术的不同表述」。
...
- **精确交集 vs 模糊匹配（前缀/子串/词元）**：选「归一化 + 前缀匹配」。精确交集对同一技术不同表述零容错（真实验证：`CMS 收集器` vs `CMS 并发标记清除` 交集为空）；子串匹配让短缩写（`gc`）虚高命中 `zgc`/`g1gc`；词元重叠依赖逐字 CJK 分词（单字噪声大）。归一化 + 前缀匹配可控、可解释、可审计。
...
**已采纳方案（模糊匹配，2026-08-15 决策）**：

- **打分端**：`overlap_score` 从精确交集改为模糊匹配——归一化（去空格/小写 + 剥前后缀泛称黑名单）+ 前缀匹配（短名 vs 全称），二值命中（呼应「不加权」）。选前缀匹配而非子串匹配：子串会让短缩写（`gc`）虚高命中 `zgc`/`g1gc` 等。
- **抽取端**：prompt 引导 LLM 输出「有明确定义且无歧义的技术名词」（全称术语或官方名称），禁止口语泛称（收集器/机制）和有歧义缩写（CMS）。
- **归一化局限（明示，2026-08-15 补）**：
  - 归一化只能剥泛称得到短名，无法把缩写展开成全称（缩写→全称映射不可维护），故「缩写 vs 全称」的对齐依赖 prompt 引导的一致性。
  - 前缀表只收「泛称限定词」（`分布式`/`持久化`——剥掉后残留仍是完整技术词，如「分布式 KV 存储」→「kv存储」）。语义承载强的复合技术名头部（动态/静态/在线/离线/内存/统一/通用）**不进前缀表**：剥掉会塌缩成无关泛词——「静态分析」「动态分析」都塌成「分析」被判同一技术、「动态规划」→「规划」、「内存屏障」→「屏障」。这些靠前缀匹配（短名 vs 全称）对齐，不靠剥前缀。
