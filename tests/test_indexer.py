...
测试策略：mock EmbeddingModelManager.encode()，验证 chunks → chunk_vecs 的组装正确性。
"""
...
class TestBuildIndex:
    ...
    def test_build_index_returns_two_tuples(self):
        """返回 (chunks, chunk_vecs) 二元组。"""
    ...
    def test_empty_paper_returns_empty(self):
        ...
        chunks, chunk_vecs = build_index(empty_paper, model)
        assert len(chunks) == 0
    ...
    def test_short_paper_produces_single_chunk(self):
        ...
        chunks, _ = build_index(short_paper, model)
        assert len(chunks) == 1
