...
class TestGetLogger:
    def test_get_logger_returns_paper_rag_child(self):
        logger = get_logger("test_module")
        assert logger.name == "paper_rag.test_module"
...
""")
    ...
    def test_setup_logging_env_level_overrides_yaml(self):
        os.environ["PAPER_RAG_LOG_LEVEL"] = "ERROR"
        try:
            setup_logging()
            logger = logging.getLogger("paper_rag")
            assert logger.level <= logging.ERROR
        finally:
            del os.environ["PAPER_RAG_LOG_LEVEL"]
    ...
    def test_setup_logging_log_dir_env_creates_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.environ["PAPER_RAG_LOG_DIR"] = str(custom_log_dir)
            try:
                setup_logging()
                assert custom_log_dir.exists()
                assert custom_log_dir.is_dir()
            finally:
                del os.environ["PAPER_RAG_LOG_DIR"]
